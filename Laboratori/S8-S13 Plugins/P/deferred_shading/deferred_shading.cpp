...
void Deferred_shading::compileAndLinkShaders()
{
    ...
    vs->compileSourceFile("gbuffer_colorAndSpecular.vert");
    fs = new QOpenGLShader(QOpenGLShader::Fragment, this);
    fs->compileSourceFile(+ "gbuffer_colorAndSpecular.frag");
    ...
    vs = new QOpenGLShader(QOpenGLShader::Vertex, this);
    vs->compileSourceFile("gbuffer_normal.vert");
    fs = new QOpenGLShader(QOpenGLShader::Fragment, this);
    fs->compileSourceFile("gbuffer_normal.frag");
    ...
    // this shader will store in the color buffer the normal information (per fragment)
    vs = new QOpenGLShader(QOpenGLShader::Vertex, this);
    vs->compileSourceFile("gbuffer_position.vert");
    fs = new QOpenGLShader(QOpenGLShader::Fragment, this);
    fs->compileSourceFile("gbuffer_position.frag");
...
bool Deferred_shading::paintGL()
{
    ...
    if (mode == 0 || mode == 3)
    {
        ...
        // Copy from the color buffer to the corresponding texture
        g.glBindTexture(GL_TEXTURE_2D, colorAndSpecularMapId);
    ...
    if (mode == 3)
    {
        ...
        // Activate and bind all textures
        ...
        // Bind deferred shading program and set uniforms
