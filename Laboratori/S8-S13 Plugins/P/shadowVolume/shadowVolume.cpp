...
bool ShadowVolume::paintGL()
{
   ...
   if (showVolume) {
	  ...
	  // Activar l'escriptura de colors
	  g.glColorMask(GL_TRUE, GL_TRUE, GL_TRUE, GL_TRUE);

	  // Activar l'escriptura al depth buffer
	  g.glDepthMask(GL_TRUE);

	  // Configurar la funció de depth per acceptar valors iguals
	  // (per evitar z-fighting amb la geometria ja dibuixada al Pas 1)
	  g.glDepthFunc(GL_LEQUAL);

	  // Configurar el test de stencil per dibuixar només on el valor és diferent de 0
	  // (dins de l'ombra)
	  g.glStencilFunc(GL_NOTEQUAL, 0, 0xFF);

	  // No modificar el stencil buffer durant el dibuix
	  g.glStencilOp(GL_KEEP, GL_KEEP, GL_KEEP);

	  // Desactivar el culling per dibuixar totes les cares
	  ...
	  // Configurar el test de stencil per dibuixar només on el valor és 0
      // (fora de l'ombra)
