...
bool Reflection::paintGL()
{
	...
	g.glClearColor(0.8,0.8,1,0);
	g.glClear(GL_DEPTH_BUFFER_BIT | GL_COLOR_BUFFER_BIT);
	
	QMatrix4x4 reflection1;
	reflection1.scale(1,1,-1);
	reflection1.translate(0,0,-2*scene()->boundingBox().min().z());
	...
	g.glClearColor(1,0.8,0.8,0);
	g.glClear(GL_DEPTH_BUFFER_BIT | GL_COLOR_BUFFER_BIT);
	 
	QMatrix4x4 reflection2;
	reflection2.scale(-1,1,1);
	reflection2.translate(-2*scene()->boundingBox().min().x(),0,0);
