...
void Draw_smooth::addVBO(unsigned int currentObject)
{
    ...
    else
    {
        ...
        for (auto&& f: obj.faces()) 
        {
            ...
            // Calculate the face normal by averaging the normals of its three vertices
            Vector N0 = Ns[f.normalIndex(0)];
            Vector N1 = Ns[f.normalIndex(1)];
            Vector N2 = Ns[f.normalIndex(2)];
            
            // Compute the averaged normal
            Vector faceNormal = (N0 + N1 + N2).normalized();
            ...
            auto pushVertex = [&](int vertexIdx, int normalIdx, int tcIdx)
            {
                ...
                colors.push_back(fabs(faceNormal.x()));
                colors.push_back(fabs(faceNormal.y()));
                colors.push_back(fabs(faceNormal.z()));
